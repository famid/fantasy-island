    ...
    public function createPayment($amount, $orderId)
    {
        try {
            ...
            $body_data = array(
                'mode' => '0011',
                'payerReference' => ' ',
                'callbackURL' => env('BKASH_CALLBACK_URL', BKASH_CALLBACK_LOCAL_URL),
                'amount' => $amount ?? 200,
                'currency' => 'BDT',
                'intent' => 'sale',
...
//        return redirect((json_decode($response)->bkashURL));
            $redirectUrl = json_decode($response)->bkashURL;

            return ['success' => True, 'data' => ['data' => $redirectUrl], 'message' => "url is generate successfully"];
        } catch (Exception $e) {
